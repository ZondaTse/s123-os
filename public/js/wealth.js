...
function sp_openSheet(idx,list){
  ...
  if (_nameDiv && !sheet.querySelector('.sp-dept-line')) {
    _nameDiv.style.cssText = 'font-size:22px;font-weight:700;color:var(--s-fg);display:inline';
    const _wrap = _nameDiv.parentNode;
    _wrap.style.cssText = 'display:flex;align-items:baseline;gap:6px;margin-bottom:6px;flex-wrap:wrap';
    const _deptEl = document.createElement('span');
    _deptEl.className = 'sp-dept-line';
    _deptEl.style.cssText = 'font-size:13px;color:var(--s-fg4);font-weight:400';
    _wrap.appendChild(_deptEl);
    const _ruleEl = document.createElement('div');
    _ruleEl.className = 'sp-rule-line';
    _ruleEl.style.cssText = 'width:100%;font-size:12px;color:var(--s-fg4);line-height:1.5;margin-top:0px';
    _wrap.parentNode.insertBefore(_ruleEl, _wrap.nextSibling);
  }
  document.getElementById('sp-d-name').textContent = p.name;
  const _d = sheet.querySelector('.sp-dept-line');
  if (_d) _d.textContent = p.dept;
  const _r = sheet.querySelector('.sp-rule-line');
  if (_r) _r.textContent = (typeof SALARY_RULES !== 'undefined' && SALARY_RULES[p.name]) || '';
  ...
  document.getElementById('sp-d-compare').innerHTML = `
    <div class="sheet-kpi-row">
      ...
      <div class="sheet-kpi" style="background:${diff==null?'var(--s-bg3)':diff>0?'var(--s-tint-red)':diff<0?'var(--s-tint-green)':'var(--s-bg3)'};display:flex;flex-direction:column;justify-content:center;min-height:72px">
        <div style="font-size:11px;color:${diff==null?'var(--s-fg4)':diff>0?'var(--s-red)':diff<0?'var(--s-green)':'var(--s-fg4)'};margin-bottom:4px">环比</div>
        <div style="font-size:20px;font-weight:700;color:${diff==null?'var(--s-fg4)':diff>0?'var(--s-red)':diff<0?'var(--s-green)':'var(--s-fg4)'};letter-spacing:-0.5px">${hasApr?(diff>0?'+¥':diff<0?'-¥':'¥')+Math.abs(diff).toLocaleString('zh-CN'):'—'}</div>
        <div style="font-size:13px;font-weight:600;color:${diff==null?'var(--s-fg4)':diff>0?'var(--s-red)':diff<0?'var(--s-green)':'var(--s-fg4)'};margin-top:2px">${hasApr?(diff>=0?'+':'')+Math.abs(pctVal)+'%':'—'}</div>
      </div>
      </div>
      </div>
      </div>
