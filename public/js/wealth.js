...
function sp_openSheet(idx,list){
  ...
  let _ruleEl = sheet.querySelector('.sp-rule-line');
  if (!_deptEl) {
    const _nameDiv = document.getElementById('sp-d-name');
    if (_nameDiv) {
      _nameDiv.style.marginBottom = '0';
      const _wrap = _nameDiv.parentNode;
      _wrap.style.cssText = 'display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px';
      _deptEl = document.createElement('div');
      _deptEl.className = 'sp-dept-line';
      _deptEl.style.cssText = 'font-size:13px;color:var(--s-fg4);font-weight:500;align-self:flex-end;padding-bottom:4px';
      _wrap.appendChild(_deptEl);
      _ruleEl = document.createElement('div');
      _ruleEl.className = 'sp-rule-line';
      _ruleEl.style.cssText = 'width:100%;font-size:12px;color:var(--s-fg4);line-height:1.5;margin:4px 0 8px';
      _wrap.appendChild(_ruleEl);
    }
  }
